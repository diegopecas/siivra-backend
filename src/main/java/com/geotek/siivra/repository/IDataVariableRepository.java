...
public interface IDataVariableRepository  extends JpaRepository<DataVariable, Long> {
    ...
    @Query(value = "SELECT auditoria FROM siivra_conf.siivrat_variable_info_alfa_v where id_variable_info_alfa = :idDataVariable and auditoria is not null ORDER BY id DESC LIMIT 1", nativeQuery = true)
